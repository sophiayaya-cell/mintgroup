...
```bash
# 线索来源（已设为公开只读端点，裸 curl 即可，用于快速验证部署存活）
curl https://www.mint-gp.com/api/sales/lead-sources

# 其他受保护端点需鉴权（dev-token 旁路 或 真实 key 二选一）：
#   curl -H "Authorization: Bearer dev-token" https://www.mint-gp.com/api/sales/accounts
#   curl -H "x-api-key: $SALES_API_KEY" https://www.mint-gp.com/api/sales/accounts
#   curl -H "Authorization: Bearer $SESSION_SECRET" https://www.mint-gp.com/api/sales/accounts

# 真实发送自检（testTo 安全模式）：把该序列到期待发的步骤只发到你指定的邮箱，
# 不碰库里真实买家的邮箱、不推进他们的序列、不写 sent 事件，纯验证渲染+送达。
# 注意：
#  - 端点路径是 /api/sales/campaigns/<序列ID>/send-now（不是 outreach/send-now）
#  - body 字段是 {"testTo":"你的邮箱"}（不是 testMode / enrollmentId）
#  - 鉴权可用 dev-token 旁路，或 Bearer <SESSION_SECRET>
curl -X POST https://www.mint-gp.com/api/sales/campaigns/cmp_001/send-now \
  -H "Authorization: Bearer dev-token" \
  -H "Content-Type: application/json" \
  -d "{\"testTo\":\"sophia.wang@mint-gp.com\"}"

# 返回示例：
# {"due":1,"sent":1,"errors":0,"queued":0,"test":true,"testTo":"sophia.wang@mint-gp.com","mode":"test","campaign_id":"cmp_001"}
#
# 收件箱应收到来自 sophia.wang@mint-gp.com 的邮件（test 模式不注入追踪像素/退订，纯渲染+送达验证）。
#
# 真实群发（发给真实买家、推进序列、记 sent 事件）：去掉 testTo 即可（空 body）：
# curl -X POST https://www.mint-gp.com/api/sales/campaigns/cmp_001/send-now \
#   -H "Authorization: Bearer dev-token" -H "Content-Type: application/json" -d '{}'
```

> ⚠️ 真实群发 `send-now` 会直接发给该序列里**所有到期真实买家**并推进序列。首次上线务必先用 `testTo` 自检，确认渲染/送达无误后再做真实群发。另外 Cloudflare Cron（每小时）也会自动触发 `sendDueSteps` 发送到期步骤——若不想被自动发送，需临时把入组联系人的 `next_step_at` 推后或把序列 `status` 置为非 active。
